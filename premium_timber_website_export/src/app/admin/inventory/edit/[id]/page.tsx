...
import { ArrowLeft, Save, Upload, X, AlertCircle, CheckCircle, Info, History, Star, Link2 } from 'lucide-react';
...
export default function EditContainerPage() {
  ...
  // Auto calculate total price when Volume (CFT) or Rate per CFT changes
  useEffect(() => {
    const cftVal = parseFloat(formData.cft as any) || 0;
    const rateVal = parseFloat(formData.ratePerCft as any) || 0;
    if (cftVal && rateVal) {
      setFormData((prev: any) => ({ ...prev, price: Math.round(cftVal * rateVal).toString() }));
    }
  }, [formData.cft, formData.ratePerCft]);

  // Images state (data URLs)
  ...
  useEffect(() => {
    const loadDetails = async () => {
      try {
        ...
        const inquiryList = await inventoryService.getInquiries();
        setInquiries(inquiryList || []);

        const details = await inventoryService.getContainerById(id);
        if (details) {
          setFormData({
            ...details,
            countryId: details.countryName || details.countryId || '',
            ratePerCft: details.ratePerCft !== undefined && details.ratePerCft !== null ? details.ratePerCft.toString() : '',
            price: details.price ? details.price.toString() : ''
          });
          setImagePreviews(details.images || []);

          // Pre-select linked inquiry if reserved
          const reservations = await inventoryService.getReservations();
          const linkedRes = (reservations || []).find(
            (r: any) => (r.containerId === details.container_number || r.containerId === details.id) && (r.status === 'approved' || r.status === 'pending')
          );
          if (linkedRes && inquiryList) {
            const matchingInq = (inquiryList || []).find(
              (inq: any) => (inq.phone && inq.phone === linkedRes.phone) || (inq.email && inq.email === linkedRes.email)
            );
            if (matchingInq) {
              setSelectedInquiryId(matchingInq.id);
            }
          }
        } else {
          setErrorMessage('Container profile not found.');
        }
      } catch (err) {
        console.error('Error fetching container profile:', err);
  ...
  // Auto calculate volume (CFT & CBM) when Length, Girth, and Logs Count change
  useEffect(() => {
    if (!autoCalculateCft || loading) return;
    const lengthVal = parseFloat(formData.avgLength as any) || 0;
    const girthVal = parseFloat(formData.avgDiameter as any) || 0;
    const countVal = parseInt(formData.logsCount as any) || 0;
    ...
    setFormData((prev: any) => ({
      ...prev,
      cft: parseFloat(calculatedCft.toFixed(2)),
      cbm: parseFloat(calculatedCbm.toFixed(3))
    }));
  }, [
    formData.avgLength, 
  ...
  const handleSave = async (isDraftMode: boolean) => {
    ...
    if (!serialId) {
      setErrorMessage('Please enter a Container ID.');
      setSaving(false);
      return;
    }

    try {
      // If the Serial ID was renamed, check for duplicates to prevent catalog collisions
      if (serialId !== id) {
      ...
      // If reserved but no inquiry selected, warn the user
      if (formData.status === 'reserved' && !selectedInquiryId) {
        setErrorMessage('Please select a customer inquiry to link with this reserved container.');
        setSaving(false);
        return;
      }
      ...
      await inventoryService.updateContainer(id, submission);

      // Handle Reservation Sync
      const reservationsStr = typeof window !== 'undefined' ? localStorage.getItem('timber_mock_reservations') || '[]' : '[]';
      const reservations = JSON.parse(reservationsStr);

      if (formData.status === 'reserved' && selectedInquiryId) {
        const inq = inquiries.find(q => q.id === selectedInquiryId);
        if (inq) {
          // Clear any active reservations for this container first
          const updatedReservations = reservations.filter(
            (r: any) => r.containerId !== formData.container_number && r.containerId !== id
          );
          
          updatedReservations.push({
            id: `res-${Date.now()}`,
            containerId: formData.container_number || id,
            customerName: inq.name || inq.customerName || 'Anonymous Client',
            companyName: inq.companyName || '',
            phone: inq.phone || '',
            email: inq.email || 'inquiry@form.com',
            city: inq.city || 'Tamil Nadu',
            date: new Date().toISOString().split('T')[0],
            status: 'approved'
          });
          localStorage.setItem('timber_mock_reservations', JSON.stringify(updatedReservations));
        }
      } else if (formData.status === 'available') {
        // If changed back to available, delete linked reservations
        const updatedReservations = reservations.filter(
          (r: any) => r.containerId !== formData.container_number && r.containerId !== id
        );
        localStorage.setItem('timber_mock_reservations', JSON.stringify(updatedReservations));
      ...
      setErrorMessage(err.message || 'Failed to update container specs.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-6 sm:p-8 md:p-10 space-y-8 select-none">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-stone-200 dark:border-zinc-900">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Edit Container specs</h1>
            <p className="text-xs text-stone-500 dark:text-zinc-450 mt-0.5">
              Modify specifications parameters or status triggers for <span className="font-semibold text-stone-950 dark:text-zinc-50">{id}</span>.
            </p>
          </div>
        </div>
      </div>
      ...
      {formData.isDraft && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-4 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs font-semibold">
          <div className="flex items-center gap-2">
            <Info className="h-4 w-4 shrink-0 text-amber-500" />
            <span>This container is currently saved as a <strong>Draft Lock</strong>. It is hidden from public buyers.</span>
          </div>
          <button
            type="button"
            onClick={() => handleSave(false)}
            className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-[10px] uppercase rounded-lg shadow-sm transition-all cursor-pointer whitespace-nowrap"
          >
            Publish Logs Live Now
          </button>
        </div>
      )}
      ...
      {success && (
        <div className="flex items-center gap-3 p-4 rounded-xl border border-green-500/20 bg-green-500/10 text-green-600 text-xs font-semibold">
          ...
          <span>Container specifications updated! Redirecting to ledger...</span>
        </div>
      )}
      ...
      <form noValidate onSubmit={(e) => { e.preventDefault(); handleSave(false); }} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="p-6 sm:p-8 rounded-2xl border border-stone-200 bg-white dark:border-zinc-900 dark:bg-zinc-900 shadow-sm space-y-6">
            ...
            {/* Pricing details */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-xl bg-stone-50/50 border border-stone-150 dark:bg-zinc-950/20 dark:border-zinc-850">
              <div>
                <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1.5">Rate per CFT (₹) *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1.5">Total Container Price (₹) (Auto/Manual)</label>
                <input
                  type="number"
                  step="1"
            ...
            {formData.isDraft ? (
              <div className="p-3.5 bg-stone-50 border border-stone-200 dark:bg-zinc-950/40 dark:border-zinc-850 rounded-xl space-y-1">
                ...
                <p className="text-[11px] text-stone-500 dark:text-zinc-400 leading-relaxed font-medium">
                  🔒 Availability status (Available, Reserved, Sold) can be managed once you Publish Logs live.
                </p>
              </div>
            ) : (
              <>
                <div>
                  <label className="block text-[9px] font-bold text-stone-400 uppercase tracking-widest mb-1.5">Availability Status</label>
                  <select
                    value={formData.status}
                    onChange={(e) => setFormData({ ...formData, status: e.target.value as any })}
                    className="w-full px-3 py-2 border border-stone-200 bg-stone-50/50 rounded-xl text-xs focus:outline-none dark:border-zinc-800 dark:bg-zinc-950/40 cursor-pointer font-semibold text-stone-850 dark:text-zinc-150"
                  >
                ...
                {formData.status === 'reserved' && (
                  <div className="p-3.5 bg-amber-500/5 border border-amber-500/20 rounded-xl space-y-2 mt-2">
                    <label className="block text-[9px] font-bold text-amber-600 dark:text-amber-500 uppercase tracking-widest">
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-amber-500/30 bg-white dark:bg-zinc-900 rounded-xl text-xs focus:outline-none dark:border-zinc-850 cursor-pointer text-stone-850 dark:text-zinc-150"
                    >
                      <option value="">-- Select CRM Client Inquiry --</option>
                      {(inquiries || []).map((inq) => (
                        <option key={inq.id} value={inq.id}>
                          {inq.name || inq.customerName || 'Anonymous'} ({inq.phone} {inq.companyName ? `- ${inq.companyName}` : ''})
                        </option>
                      ))}
                    </select>
                    <p className="text-[10px] text-stone-450 dark:text-zinc-500">
                      Linking an inquiry automatically creates and approves a Reservation ledger record for this customer.
                    </p>
                  </div>
                )}
              </>
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-grow px-3 py-1.5 border border-stone-250 bg-stone-50/50 rounded-xl text-[10px] focus:outline-none focus:border-amber-500 dark:border-zinc-850 dark:bg-zinc-950/45 text-stone-850 dark:text-zinc-150"
                />
                <button
                  type="button"
